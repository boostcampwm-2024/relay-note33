...
> <더글라스 크락포드>
 ...
 "Don't reinvent the wheel"이라는 격언처럼 개발자들은 이미 존재하는 프로그램을 다시 만드는 일은 쓸모 없다고 생각하는 경향이 있습니다. 저 역시도 개발할 때, 혼자 만들 필요가 없다고 생각하고 실행하지 않는 경우가 많았습니다.
 ...
 마지막으로 책에서는 굴러가는 바퀴를 만드는 것은 지금의 밥값을 하는 일이라면, 더 잘 굴러가는 바퀴를 만드는 것은 미래의 몸값을 만드는 것이 될 수 있다고 합니다. 단순히 구현하는 것을 넘어서 제대로 이해하고 더 잘 구현하는 것을 목표로 개발한다면 지속 가능한 개발자로 성장할 수 있을 것이라는 생각이 듭니다.
